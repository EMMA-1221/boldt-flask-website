...
def data_import():
    ...
    #extract data
    P3_extra = pd.DataFrame(columns=['Pri_D','Pri_E','Pri_P','Pri_C','Env_D','Env_E', \
                                     'Env_P','Env_C','High_Trait','Low_Trait','Decision_Style','Leadership_Style', \
    ...
    for record in P3['Profile Report']:
        ...
        P3_extra.loc[len(P3_extra)] = [priD,priE,priP,priC,envD,envE,envP,envC,High, \
                                       Low,Dec,Lead,Energy,Stress,Stress_num,Energy_num,Proact_num,Self_num]
    #add to dataset    
    ...
    P3 = P3[['Name', 'First Name', 'Last Name','Survey_Date','Pri_D','Pri_E','Pri_P','Pri_C', \
             'Cons','Env_D','Env_E','Env_P','Env_C','High_Trait','Low_Trait','Decision_Style','Leadership_Style', \
    ...
    for index, test in P3.iterrows():
        flex = abs(test['Pri_D'] - test['Env_D']) + abs(test['Pri_E'] - test['Env_E']) + \
        abs(test['Pri_P'] - test['Env_P']) + abs(test['Pri_C'] - test['Env_C'])
